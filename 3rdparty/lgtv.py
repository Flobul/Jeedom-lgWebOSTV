...
class LGTVClient(WebSocketClient):
    def __init__(self, hostname=None):
        ...
        if os.path.exists(os.path.expanduser("/var/www/html/plugins/lgWebOSTV/3rdparty/lgtv.json")):
            f = open(os.path.expanduser("/var/www/html/plugins/lgWebOSTV/3rdparty/lgtv.json"))
    ...
    def __store_settings(self):
        ...
        f = open(os.path.expanduser("/var/www/html/plugins/lgWebOSTV/3rdparty/lgtv.json"), "w")
    ...
    def doit(self, message, callback=None):
        self.__send_command("", "request", "ssap://" + message)
        #self.__send_command("", "request", "ssap://" + message, {"message": message}, callback)

    def sendkey(self, message, callback=None):
        self.__send_command("", "request", "ssap://com.webos.service.ime/insertText", {"text": message}, callback)
...
def usage(error=None):
    ...
    print "Modder / Jeedom integration: Bobox59 <masquelierb@gmail.com>"
    print "Usage: lgtv <command> [parameter]"
    print
    print "Available Commands:"

    print "  scan"
    print "  auth                  Hostname/IP    Authenticate and exit, creates initial config /var/www/html/plugins/lgWebOSTV/3rdparty/lgtv.json"
